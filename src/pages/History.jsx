...
function History() {
    ...
    const [historyQuizCheck,setHistoryQuizCheck] = useState(false);
    ...
    useEffect(() => {
        const getHistoryQuiz = async () => {
            const result = await axios.get("https://quiz-api-delta.vercel.app/history");
    ...
    const resetHandler = ()=>{
        ...
        setCount(0);
        setScore(0);
        titleRef.current.innerHTML = "History Quiz";
    }  
  return (
    <div className="bg-slate-900">
        {
        loading ? <Spinner/> : !historyQuizCheck ? <Spinner/> : (
            <div className="h-screen">
                <p className="text-right text-2xl text-white mr-3">Score  {score} </p>
                <h1 className="mt-12 text-white text-6xl flex justify-center items-center mb-5" ref={titleRef}>History Quiz</h1>
