...
function Geo() {
    const [geoQuiz,setGeoQuiz] = useState([]);
    const [geoQuizCheck,setGeoQuizCheck] = useState(false);
    ...
    useEffect(() => {
        const getGeoQuiz = async () => {
            const result = await axios.get("https://quiz-api-delta.vercel.app/geography");
    ...
    useEffect(() => {
        function selectRandomElements(array, num) {
            ...
            setGeoQuizCheck(true);
            setCorrect(arrayCopy[count].answer);
            return arrayCopy.slice(0, num);
        }
        if (geoQuiz.length > 0) {
            setRandomElementsSet(selectRandomElements(geoQuiz, 10));
        }
    ...
    const changeHandler = () => { 
        if(count === 9){ 
            setCorrect(null);
            let mark = parseInt(score) +10;
            titleRef.current.innerHTML = "You have completed the quiz with "+mark+" points";
            return;
         }
        setCount((count)=>{
    ...
    const resetHandler = ()=>{
        ...
        setCount(0);
        setScore(0);
        titleRef.current.innerHTML ="Geography Quiz";
    }  
  return (
    <div className="bg-slate-900">

        {
        loading ? <Spinner/> : !geoQuizCheck ? <Spinner/> : (
            <div className="h-screen">
                <p className="text-right text-2xl text-white mr-3">Score  {score} </p>
                <h1 className="mt-12 text-white text-6xl flex justify-center items-center mb-5" ref={titleRef}>Geography Quiz</h1>
